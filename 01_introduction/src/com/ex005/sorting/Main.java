...
//        Bird uu = (Bird) o;
//        if (this.size != uu.size){
        int i = this.size - ((Bird) o).size;

        if (i != 0) {
            return i; //если не 0, а + или - то вернуть разницу чисен, код сам поймёт.
        } else { //первый равен и делаем сортировку дальше!!
            i = this.maxAge - ((Bird) o).maxAge;
            if (i != 0) {
                return i; //если не 0, а + или - то вернуть разницу чисен, код сам поймёт.
            } else {
...
public class Main {
    public static void main(String[] args) {
        ...
        Bird n2 = new Bird(23456, "ttrtert", 123);
